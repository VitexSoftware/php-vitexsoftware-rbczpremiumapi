...
class ApiClient extends \GuzzleHttp\Client
{
    ...
    public function __construct(array $config = [])
    {
        ...
        $globalRateLimitPerSecond = (int) ($config['global_rate_limit_per_second'] ?? \Ease\Shared::cfg('RBAPI_GLOBAL_RATE_LIMIT_PER_SECOND', 0));
        $maxWaitSeconds = (int) ($config['rate_limit_max_wait_seconds'] ?? \Ease\Shared::cfg('RBAPI_RATE_MAX_WAIT_SECONDS', 300));
        $this->rateLimiter = new RateLimiter($limitStore, $waitMode, $lockDir, $globalRateLimitPerSecond, $maxWaitSeconds);
    ...
    /**
     * Enforce the host-wide, self-tracked rate limit shared by every certificate
     * using this store (see {@see RateLimiter::checkGlobalBeforeRequest()}).
     *
     * Serializes the check against concurrent processes via a dedicated lock
     * keyed by {@see RateLimiter::GLOBAL_CLIENT_ID} (distinct from the
     * per-certificate lock already held around this call), then releases it
     * immediately — only the check-and-increment is serialized, not the
     * outbound HTTP call itself.
     *
     * @throws RateLimitExceededException if the host-wide cap is exceeded and wait mode is disabled
     */
    private function throttleGlobally(): void
    {
        $globalLock = $this->rateLimiter->acquireLock(RateLimiter::GLOBAL_CLIENT_ID);

        try {
            $this->rateLimiter->checkGlobalBeforeRequest();
        } finally {
            $this->rateLimiter->releaseLock($globalLock);
        }
    }
